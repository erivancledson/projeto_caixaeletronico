...
if(isset($_SESSION['banco']) && empty($_SESSION['banco']) == false) {
	...
	$sql->bindValue(":id", $id);
	$sql->execute();
     //se existir o usuario com esse id mostra os dados dele, se não ele vai para a pagina de login
	if($sql->rowCount() > 0) {
		$info = $sql->fetch();
	} else {
		...
		if($sql->rowCount() > 0) {
			foreach($sql->fetchAll() as $item) {
				...
				<tr>
					...
					<td>
					<!--0 quando é deposito e 1 quando é uma retirada -->
						<?php if($item['tipo'] == '0'): ?>
